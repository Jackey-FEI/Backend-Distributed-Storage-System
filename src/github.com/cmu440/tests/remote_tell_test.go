...
func setupTestRemoteTell(t *testing.T) []*actor.ActorSystem {
	...
	systems := make([]*actor.ActorSystem, 4)
...
func runTestRemoteTellOrder(t *testing.T, checkOrder bool, deadline time.Duration, desc string) {
	...
	reportRef, reportCh := systems[2].NewChannelRef()
	receiverRef := systems[2].StartActor(newReceiveActor)
	systems[2].Tell(receiverRef, ReceiveActorInit{
	...
	// senderRef2 := systems[1].StartActor(newSendActor)
	// systems[1].Tell(senderRef2, SendActorCmd{
	// 	Target: receiverRef,
	// 	Count:  remoteTellCount,
	// })
